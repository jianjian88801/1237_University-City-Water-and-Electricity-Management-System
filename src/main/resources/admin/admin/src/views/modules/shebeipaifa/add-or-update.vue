<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="是否派发" prop="shifoupaifa">
          <el-select v-model="ruleForm.shifoupaifa" placeholder="请选择是否派发">
            <el-option
                v-for="(item,index) in shifoupaifaOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="是否派发" prop="shifoupaifa">
	      <el-input v-model="ruleForm.shifoupaifa"
                placeholder="是否派发" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
      ...
      <el-col :span="12">
        <el-form-item class="date" v-if="type!='info'" label="派发时间" prop="paifashijian">
            <el-date-picker
                format="yyyy 年 MM 月 dd 日"
                value-format="yyyy-MM-dd"
                v-model="ruleForm.paifashijian" 
                type="date"
                placeholder="派发时间">
            </el-date-picker> 
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.paifashijian" label="派发时间" prop="paifashijian">
              <el-input v-model="ruleForm.paifashijian" 
                placeholder="派发时间" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      </el-row>
...
export default {
  ...
  created() {
	this.ruleForm.paifashijian = this.getCurDate()
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.xingming = true;
          ...
          if(o=='shifoupaifa'){
            this.ruleForm.shifoupaifa = obj[o];
	    this.ro.shifoupaifa = true;
            continue;
          }
	    ...
	    this.ro.shuliang = true;
          ...
          if(o=='paifashijian'){
            this.ruleForm.paifashijian = obj[o];
	    this.ro.paifashijian = true;
            continue;
          }
        }
      }
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
            ...
            this.shifoupaifaOptions = "是,否".split(',')
    },
    // 多级联动参数
    info(id) {
      this.$http({
        url: `shebeipaifa/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `shebeipaifa/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.shebeipaifaCrossAddOrUpdateFlag = false;
                  this.parent.search();
